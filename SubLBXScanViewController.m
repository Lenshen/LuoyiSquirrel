...
- (void)drawTitle
{
    if (!_topTitle)
    {
        ...
        _topTitle.text = @"扫一扫";
        _topTitle.font =[UIFont systemFontOfSize:19];
...
- (void)drawTopview
{
    ...
    dissButton.frame = CGRectMake(10, 0, 64, 64);
    [dissButton setImage:[UIImage imageNamed:@"black"] forState:UIControlStateNormal];
    [dissButton addTarget:self action:@selector(dissmissViewController) forControlEvents:UIControlEventTouchUpInside];
    dissButton.imageEdgeInsets = UIEdgeInsetsMake(25, 0, 20, 4);
    [self.topView addSubview:dissButton];
